...
import com.group25.ebnisina.manageresults.repository.ReportRepository;
...
public class ResultService {
    ...
    public List<ReportResult> getMinAndMaxResultsOfParameters(int laboratorian_id) {
        return reportRepository.getMinAndMaxResultsOfParameters(laboratorian_id);
    }
    ...
    public void addResultToTestRequest(List<Result> result) {
        ...
        result.forEach(row -> resultRepository.addResultToTestRequest(row.getScore(), row.getTest_type_id(), row.getParameter_name(),
                testRequestId));
